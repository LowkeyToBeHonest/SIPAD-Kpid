async function request(action,options={}){
	...
	let response;
	try{response=await fetch(CONFIG.WEBAPP+(options.method==='POST'?'':'?'+query),options.method==='POST'?{method:'POST',redirect:'follow',credentials:'omit',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({...options.body,action,token})}:undefined);}catch(error){throw new Error('Web App tidak dapat dihubungi. Pastikan URL deployment benar dan aksesnya Anyone. Detail: '+error.message);}
	let data;
